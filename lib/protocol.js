...
Protocol.prototype.$parseResponse = function $parseResponse (raw) {
    var parts = raw.split(this.$DELIMITER);
    var response = {};

    if (!parts.length || parts[0] !== 'response') {
        return response;
    }

    response.id = parts[1];
    response.command = parts[2];
    response.body = parts[3];
...
Protocol.prototype.$serializeRequest = function $serializeRequest (dto) {
    var request = ['request', dto.id, dto.command, dto.body];
    var result = request.join(this.$DELIMITER);
...
Protocol.prototype.serialize = function serialize (type, dto) {
    ...
    switch (type) {
        ...
        case 'request':
            result = this.$serializeRequest(dto);
        break;
    }
